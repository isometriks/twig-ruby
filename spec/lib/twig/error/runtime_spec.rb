...
describe Twig::Error::Runtime do
  describe '#initialize' do
    let(:message) { 'Runtime error message' }
    let(:lineno) { 20 }
    ...
    it 'sets the error message' do
      expect(error.to_s).to include('Runtime error messag')
    end
    ...
    it 'includes the line number in the error message' do
      expect(error.to_s).to include('line 20')
    end
  end

  describe 'with default parameters' do
    subject(:error) { described_class.new('Runtime error message') }
