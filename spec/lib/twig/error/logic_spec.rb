...
describe Twig::Error::Logic do
  describe '#initialize' do
    let(:message) { 'Logic error message' }
    let(:lineno) { 15 }
    ...
    it 'sets the error message' do
      expect(error.to_s).to include('Logic error messag')
    end
    ...
    it 'includes the line number in the error message' do
      expect(error.to_s).to include('line 15')
    end
  end

  describe 'with default parameters' do
    subject(:error) { described_class.new('Logic error message') }
